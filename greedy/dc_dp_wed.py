...
def merge(A, left, mid, right):
    ...
    while i <= mid and j <= right:
        if A[i] <= A[j]:
            ...
            j += 1
            k += 1
    # 4-1. 왼쪽 부분 리스트에 요소가 남아 있는 경우 임시 리스트에 모두 복사
    while i <= mid:
        sorted_list[k] = A[i]
        i += 1
        k += 1
    # 4-2. 오른쪽 부분 리스트에 요소가 남아 있는 경우 임시 리스트에 모두 복사
    while j <= right:
        sorted_list[k] = A[j]
        j += 1
        k += 1
...
def merge_sort(A, left, right):
    if left < right : # 항목이 두 개 이상인 경우에만 분할
        mid = (left + right) // 2
        merge_sort(A, left, mid)    # 왼쪽 서브리스트 병합정렬
        merge_sort(A, mid+1, right) # 오른쪽 서브리스트 병합정렬
        merge(A, left, mid, right)  # 정렬된 두 개의 부분리스트를 병합
...
#===================================================
#알고리즘 설계 전략: 동적계획법 (DP: Dynamic Programing)
#===================================================
