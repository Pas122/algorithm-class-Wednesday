...
def search_bst(n, key):
    # 키(key)값으로 노드를 찾는 순환 탐색 : 사전에서 단어를 찾는 것 -> 알파벳 순서로 바로 찾아감
    if n is None:
        return None # failed
    elif key == n.key: # 키가 일치하면
        return n # 해당 노드 반환
    elif key < n.key:
        return search_bst(n.left, key) # 외쪽 서브트리로 재귀적 탐색
    else:
...
def delete_bst(root, key):
    ...
    # (2) 삭제할 노드가 오른쪽 서브트리에 있는지 검사
        ...
        while succ.left is not None:
            succ = succ.left
        # (4) 후계자의 키와 값을 현재 삭제할 노드(root)로 복사
        root.key = succ.key
