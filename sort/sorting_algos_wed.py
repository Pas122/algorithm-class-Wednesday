...
def quick_sort(arr, left, right): # 퀵정렬
    ...
    def partition(A, left, right): # 내부 분할 함수
        ...
        while low <= high: # 엇갈릴 때까지 반복
            ...
            while high >= left and A[high] > pivot: # 피벗보다 작거나 가은 값(<= pivot) 찾기
            ...
            if low <= high: # 이 둘은 서로 반대편에 있어야 할 값이므로, 서로 교환
                ...
                break
        # 루프를 빠져나오면 high는 "피벗 이라(<= pivot) 마지막 위치"를 가리킴
...
def list_sorted(arr): # 리스트 arr의 sort 매서드
...
for n in sizes:
    ...
    data = []
    for _ in range(n):
        data.append(random.randint(1, 99999)) # 1~99999 사이에 있는 정수 난수 1개 반환=> 균등분포
